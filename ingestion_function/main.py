...
def _download_drive_file_as_bytes(drive_service: Resource, file_id: str) -> tuple[io.BytesIO, str]:
...
def _transform_dataframe(file_bytes: io.BytesIO, file_type: str, file_name: str, config: Dict[str, Any], bank: str, account_type: str) -> pd.DataFrame:
    """Transforma los bytes de un archivo en un DataFrame limpio y enriquecido."""
    try:
        if file_type == '.csv':
            df = pd.read_csv(
                file_bytes, skiprows=config.get('skip_rows', 0), skipfooter=config.get('skip_footer', 0),
            ...
            logging.error(f"Formato de archivo no soportado para '{file_name}': {file_type}")
            return pd.DataFrame()
    except Exception as e:
...
def _get_subfolder_ids(drive_service: Resource, parent_id: str) -> Dict[str, str]:
    ...
    if IN_PROGRESS_FOLDER not in folder_ids:
        ...
        metadata = {'name': IN_PROGRESS_FOLDER, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [parent_id]}
        created_folder = drive_service.files().create(body=metadata, fields='id').execute()
...
def _process_account_folder(drive_service: Resource, account_folder: Dict[str, str], bank_name: str, config: Dict[str, Any]) -> int:
    ...
    pending_id, processed_id, in_progress_id = (
        subfolder_ids.get(PENDING_FOLDER), subfolder_ids.get(PROCESSED_FOLDER), subfolder_ids.get(IN_PROGRESS_FOLDER))
    
    if not all([pending_id, processed_id, in_progress_id]):
        logging.error(f"Falta estructura de carpetas críticas en {bank_name}/{account_type_name}. Se omite.")
        return 0

    files = drive_service.files().list(q=f"'{pending_id}' in parents and trashed=false", fields="files(id, name)").execute().get('files', [])
    if not files:
        return 0
    ...
    for file_item in files:
        ...
        try:
            ...
            file_bytes, file_type, file_name = _download_drive_file_as_bytes(drive_service, file_id)
            logging.info(f"Procesando archivo: {file_name} (ID: {file_id})")
            
            clean_df = _transform_dataframe(file_bytes, file_type, file_name, config, bank_name, account_type_name)
            
            if clean_df.empty:
                raise ValueError(f"DataFrame vacío para '{file_name}' tras la transformación.")
            ...
            storage_client.bucket(GCS_BUCKET_NAME).blob(gcs_path).upload_from_string(jsonl_data, 'application/jsonl')
            ...
            logging.error(f"Error procesando el archivo ID {file_id}. Se devolverá a 'pending'. Error: {e}")
            try:
                _move_file_in_drive(drive_service, file_id, in_progress_id, pending_id)
            except Exception as move_error:
                logging.critical(f"¡FALLO IRRECUPERABLE! No se pudo devolver el archivo ID {file_id} a 'pending'. Error: {move_error}")
    return successful_files

def _trigger_dbt_job(dbt_args: list[str] = None):
    """
    ...
    try:
        logging.info(f"Iniciando ejecución del job de dbt con los argumentos: {dbt_args}")
...
def ingest_bank_statements_pubsub(cloud_event: CloudEvent):
    ...
    try:
        ...
        logging.info("Mensaje de Pub/Sub vacío o malformado. Usando 'dbt run' por defecto.")
    ...
    try:
        ...
        logging.critical(f"Error fatal durante la ejecución principal: {e}", exc_info=True)
    ...
    if total_files_processed > 0 or "--full-refresh" in dbt_command_args:
        ...
        _trigger_dbt_job(dbt_args=dbt_command_args)
    else:
        logging.info("No se procesaron archivos nuevos y no se solicitó full-refresh. El job de dbt no se ejecutará.")
